...
import com.mapbox.mapboxsdk.tileprovider.modules.MultiMBTilesFileArchive;
 ...
 */
public class MultiMBTilesLayer extends TileLayer implements MapViewConstants, MapboxConstants {

    private static final String TAG = "MultiMBTilesLayer";
    MultiMBTilesFileArchive mMultiMBTilesFileArchive;
     ...
     */
    public MultiMBTilesLayer(final Context context, final String url, final int mapIdentifier) {
        super(url.substring(url.lastIndexOf('/') + 1, url.lastIndexOf('.')), url);
        initialize(url, context, mapIdentifier);
    }
     ...
     * @param url path to a MBTiles file
     */
    public MultiMBTilesLayer(final String url, final int mapIdentifier) {
        this(null, url, mapIdentifier);
    }
     ...
     */
    public MultiMBTilesLayer(final File file, final int mapIdentifier) {
        super(file.getName(), file.getAbsolutePath());
        initialize(file, mapIdentifier);
    }
     ...
     */
    public MultiMBTilesLayer(final SQLiteDatabase db, final int mapIdentifier) {
        super(getFileName(db.getPath()), db.getPath());
        initialize(db, mapIdentifier);
    }
     ...
     * @param file
     */
    private void initialize(File file, int mapIdentifier) {
        if (file != null) {
            mMultiMBTilesFileArchive = MultiMBTilesFileArchive.getDatabaseFileArchive(file, mapIdentifier);
        }

        if (mMultiMBTilesFileArchive != null) {
            mMaximumZoomLevel = mMultiMBTilesFileArchive.getMaxZoomLevel();
            mMinimumZoomLevel = mMultiMBTilesFileArchive.getMinZoomLevel();
            mName = mMultiMBTilesFileArchive.getName();
            mDescription = mMultiMBTilesFileArchive.getDescription();
            mAttribution = mMultiMBTilesFileArchive.getAttribution();
            mBoundingBox = mMultiMBTilesFileArchive.getBounds();
            mCenter = mMultiMBTilesFileArchive.getCenter();
        }
    }
     ...
     */
    private void initialize(final SQLiteDatabase db, int mapIdentifier) {
        if (db != null) {
            mMultiMBTilesFileArchive = new MultiMBTilesFileArchive(db, mapIdentifier);
        }
     ...
     * Reads and opens a MBTiles file given by url and loads its tiles into this layer.
     */
    private void initialize(String url, final Context context, final int mapIdentifier) {
        initialize(getFile(url, context), mapIdentifier);
    }
    ...
    public void detach() {
        if (mMultiMBTilesFileArchive != null) {
            mMultiMBTilesFileArchive.close();
            mMultiMBTilesFileArchive = null;
        }
    }
    ...
    public CacheableBitmapDrawable getDrawableFromTile(final MapTileDownloader downloader,
        ...
        if (mMultiMBTilesFileArchive != null) {
            InputStream stream = mMultiMBTilesFileArchive.getInputStream(this, aTile);
            if (stream != null) {
